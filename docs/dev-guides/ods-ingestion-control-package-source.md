# ods_ingestion_control Package Source

This file contains the `ods_ingestion_control` Python package source, written out file by file so it can be used on another computer where the repository is not available.

This package is required by `ods_pipeline`. The `ods_pipeline` modules import it as `import ods_ingestion_control as control`, so the public names exported by `ods_ingestion_control/__init__.py` must be present at package level.

## Public Package-Level Names

These names are exported by `ods_ingestion_control/__init__.py` and are safe for `ods_pipeline` or job code to call:
...
Names beginning with `_`, such as `_call` and `_json`, are internal implementation helpers inside `control.py`; `ods_pipeline` does not call them.

## File Tree

```text
ods_ingestion_control\__init__.py
ods_ingestion_control\control.py
```
