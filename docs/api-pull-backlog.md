...
| 7 | Direct API → Kafka pull pattern (event-style, no Glue / Parquet) | P2 | TBD | **Built (slice 1).** Full design at `docs/api-pull-direct-kafka-design.md`. Runner + dispatcher in `ods_pipeline/ingest/api_pull_kafka/runner.py` and `airflow/dags/dag_api_pull.py`; live E2E in `tests/integration/test_api_pull_kafka_e2e_live.py`. Long-running runner mode and S3-sink-lag check still deferred. |
| 8 | File → Direct-Postgres pattern (skip Kafka entirely) | P2 | TBD | **Built.** Full design at `docs/file-direct-postgres-design.md`. Lands `delivery=direct_postgres` for `source_type=s3_batch`. New Glue job `glue/jobs/ods_postgres_write.py` reads curated Parquet and writes Postgres via append OR stage-and-merge upsert keyed on `dataset_config.key_fields`; inline canonicalize re-attaches ODS metadata for `is_canonical=false`. New DAG `dag_ingest_direct_postgres` skips Kafka + canonicalize + sink wait; `dag_drop_to_raw` routes by `delivery`. T2 recon recognises `direct_postgres` runs as authoritative source. Live E2E in `tests/integration/test_file_direct_pg_e2e_live.py` covers both write modes (3 rows + overlap upsert / 5 rows append). Non-canonical direct_postgres path is wired but not yet covered by a test dataset. |
| 9 | Extend unit-test coverage for `glue/jobs/`, `services/`, and `airflow/dags/common/` | P1 | TBD | **Open.** Coverage gate now spans these packages (`pyproject.toml` source list + `--cov-fail-under=40` in `.github/workflows/unit.yml`). Baseline at gate-introduction: `ods_pipeline` ~74%, `airflow/dags/common` ~46% (recon=92%, yaml_loader=79%, connect_admin=65%, kafka_admin=0%, connector_provisioner=0%), `glue/jobs` ~10% (canonicalize=55%, utils=70%, every other Spark job at 0% — `dq.py`, `ods_canonicalize.py`, `ods_ingestion.py`, `ods_merge.py`, `ods_postgres_write.py`, `ods_s3_publish.py`, `ods_stage.py`), `services/event_api/main.py` 0%. The Spark jobs are testable behind the `requires_spark` marker; `services/event_api/main.py` is testable today with FastAPI's `TestClient`. `airflow/dags/common/kafka_admin.py` and `connector_provisioner.py` only need their helpers exercised against fakes. Goal: lift each package's measured coverage by one band (glue/jobs to >40%, services to >60%, dags/common to >70%) and bump `--cov-fail-under` accordingly. |
