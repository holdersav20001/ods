...
| 7 | Direct API → Kafka pull pattern (event-style, no Glue / Parquet) | P2 | TBD | Designed; not built. Full design at `docs/api-pull-direct-kafka-design.md`. Today api_pull goes API → S3 raw archive → Glue → Parquet → raw Kafka → ... — chosen by design doc §2 to reuse the file pipeline. The direct path: long-running poller publishes per-record direct to raw Kafka; Kafka Connect S3 sink archives in parallel; recon flips from "fetched == archived" counts to offset-delta-vs-sink-lag. Build only if a real source needs sub-minute latency. |
