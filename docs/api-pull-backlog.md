...
| 8 | File → Direct-Postgres pattern (skip Kafka entirely) | P2 | TBD | Designed; not built. Full design at `docs/file-direct-postgres-design.md`. Adds `delivery=direct_postgres` for `source_type=s3_batch` datasets that don't need a Kafka leg. Reuses Glue ingestion + curated Parquet, adds `glue/jobs/ods_postgres_write.py` (inline canonicalize + Spark JDBC stage-and-merge), drops Kafka publish + Connect sink. Build only when a real reference / lookup / pre-canonicalized batch dataset asks for it. |
