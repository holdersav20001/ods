...
<div class="max-w-screen-xl mx-auto px-6 py-8 space-y-6">
  ...
  <div class="card p-6 fade-in">
    ...
    <div id="pipeline-flow" class="flex items-stretch gap-8 overflow-x-auto pb-2">
      <!-- populated by JS -->
...
function render(data) {
  const { file, runs, stages, affected } = data;
  ...
  const nodes = [
    ...
    },
    {
      ...
      extra: ingestRun ? fmtDuration(ingestRun.started_at, ingestRun.ended_at) : '',
    },
    {
      icon: '📦',
      label: 'S3 Curated',
    ...
    },
    {
      icon: '📡',
      label: 'Kafka',
      sub: publishRun ? (publishRun.kafka_topic || '—') : '—',
    ...
    },
    {
      icon: '🐘',
      label: 'Postgres',
      sub: `policy: ${fmt(affected.insurance_policy)} · history: ${fmt(affected.insurance_policy_history)}`,
      count: fmt(affected.insurance_policy + affected.insurance_policy_history),
  ...
  document.getElementById('pipeline-flow').innerHTML = nodes.map((n, i) => `
    <div class="pipeline-node card flex-shrink-0 w-40 p-4 flex flex-col items-center text-center gap-1 ${i < nodes.length-1 ? 'mr-4' : ''}">
  ...
  const stageIcon = { ingest:'📥', read_parquet:'📦', dq_check:'🔬', publish:'📤', merge:'🔀' };
    ...
    const html = runs.map((r, ri) => {
      ...
      const stageRows = runStages.map(s => `
        <tr class="bg-slate-50 border-b border-slate-100 align-top text-xs">
          ...
          <td class="py-2 text-right text-slate-400 whitespace-nowrap">${fmtDuration(s.started_at, s.ended_at)}</td>
        </tr>
      `).join('');
